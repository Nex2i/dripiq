...
async function processTimeout(job: Job<TimeoutJobPayload>): Promise<TimeoutJobResult> {
  ...
  try {
    ...
    if (campaign && campaign.planJson && campaign.currentNodeId) {
      ...
      logger.warn('Could not process campaign transition - missing campaign data', {
        tenantId,
        campaignId,
    ...
    logger.error('Failed to process timeout job', {
      jobId: job.id,
      campaignId,
      nodeId,
      eventType,
...
const timeoutWorker = getWorker<TimeoutJobPayload, TimeoutJobResult>(
  QUEUE_NAMES.campaign_execution,
  async (job: Job<TimeoutJobPayload>) => {
    if (job.name === 'timeout') {
