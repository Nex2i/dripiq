...
> {
  ...
  // Concrete CRUD
  async create(data: NewSendRateLimit): Promise<SendRateLimit> {
    const [result] = await this.db.insert(this.table).values(data).returning();
    return result as SendRateLimit;
  }

  async createMany(data: NewSendRateLimit[]): Promise<SendRateLimit[]> {
    return (await this.db.insert(this.table).values(data).returning()) as SendRateLimit[];
  }

  async findById(id: string): Promise<SendRateLimit> {
    const results = await this.db.select().from(this.table).where(eq(this.table.id, id)).limit(1);
    if (!results[0]) throw new NotFoundError(`SendRateLimit not found with id: ${id}`);
    return results[0];
  }

  async findByIds(ids: string[]): Promise<SendRateLimit[]> {
    if (ids.length === 0) return [];
    return (await this.db
      .select()
      .from(this.table)
      .where(inArray(this.table.id, ids))) as SendRateLimit[];
  }

  async findAll(): Promise<SendRateLimit[]> {
    return (await this.db.select().from(this.table)) as SendRateLimit[];
  }

  async updateById(
    id: string,
    data: Partial<NewSendRateLimit>
  ...
  async deleteById(id: string): Promise<SendRateLimit | undefined> {
    const [result] = await this.db.delete(this.table).where(eq(this.table.id, id)).returning();
    return result as SendRateLimit | undefined;
  }

  async deleteByIds(ids: string[]): Promise<SendRateLimit[]> {
    if (ids.length === 0) return [];
    return (await this.db
      .delete(this.table)
