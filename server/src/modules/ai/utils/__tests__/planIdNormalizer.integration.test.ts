...
describe('Plan ID Normalizer Integration Tests', () => {
  describe('Real-world Campaign Plan Processing', () => {
    it('should handle the provided example campaign plan correctly', () => {
      ...
      expect(aiGeneratedPlan.nodes.map(n => n.id)).toEqual([
        "email_intro", "wait_click", "email_bump_1", "stop"
      ]);
    ...
    it('should handle campaign plan service integration workflow', () => {
        ...
        nodes: [
          {
            ...
            transitions: [
              { on: "opened", to: "followup", within: "PT24H" }
            ]
          },
          {
            id: "followup",
            ...
            transitions: [
              { on: "opened", to: "complete", within: "PT72H" }
            ]
          },
          {
            id: "complete",
      ...
      expect(processedPlan.startNodeId).toMatch(/^c[a-z0-9]{23,}$/);
    ...
    it('should handle multiple campaigns with same node names without conflicts', () => {
      ...
      expect(normalized1.nodes[0].subject).toBe("Campaign 1 Subject");
      expect(normalized2.nodes[0].subject).toBe("Campaign 2 Subject");
    });
    ...
    it('should maintain execution order and flow logic', () => {
        ...
        nodes: [
          {
            ...
            transitions: [
              ...
              { on: "bounced", to: "cleanup", after: "PT0S" }
            ]
          },
          {
            id: "engagement_check",
      ...
      complexPlan.nodes.forEach((original, index) => {
        nodeMap.set(original.id, normalized.nodes[index].id);
      });
      ...
      expect(welcomeNode.transitions!.find(t => t.on === "bounced")!.to).toBe(cleanupNodeId);

      // Verify timing and event types are preserved
      expect(welcomeNode.transitions!.find(t => t.on === "opened")!.within).toBe("PT2H");
      expect(welcomeNode.transitions!.find(t => t.on === "no_open")!.after).toBe("PT24H");
      expect(welcomeNode.transitions!.find(t => t.on === "bounced")!.after).toBe("PT0S");
    });
  });
  ...
  describe('Error Recovery and Edge Cases', () => {
    it('should handle malformed plans gracefully', () => {
      ...
      expect(normalized.nodes[0].id).toMatch(/^c[a-z0-9]{23,}$/);
