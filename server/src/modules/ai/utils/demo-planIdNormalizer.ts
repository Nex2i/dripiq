...
export function demonstrateNormalizer() {
  ...
  console.log('Node IDs:', originalPlan.nodes.map(n => n.id));
  ...
  console.log('Node IDs:', normalizedPlan.nodes.map(n => n.id));
...
export function processRawPlanExample(rawPlan: CampaignPlanOutput) {
  ...
  console.log(`  - Total Transitions: ${normalizedPlan.nodes.reduce((sum, n) => sum + (n.transitions?.length || 0), 0)}`);
