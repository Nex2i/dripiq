...
export default function EmailSignatureEditor({
  ...
  const renderPreview = () => {
    ...
    if (isHtml) {
      ...
      return (
        <div
          className="p-4 border border-gray-200 rounded-md bg-white whitespace-pre-wrap"
        >
          {value}
        </div>
      )
    ...
    <div className="space-y-4">
        ...
        {!showPreview ? (
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">
              You can enter plain text or HTML. Common HTML tags like &lt;br&gt;, &lt;p&gt;, &lt;strong&gt;, &lt;em&gt;, and &lt;a&gt; are supported.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
      ...
      {value.length >= maxLength * 0.9 && (
        <div className="p-3 rounded-lg border border-yellow-200 bg-yellow-50 text-yellow-800 text-sm">
          You're approaching the character limit. Consider shortening your signature.
        </div>
      )}
    </div>
  )
